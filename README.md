...
- **Text to Video Generation**: Generate videos from text prompts using various models
- **Image to Video Generation**: Create videos from reference images
- **Video Avatar Generation**: Generate talking head videos with audio synchronization
- **Multi-GPU Support**: Enable even usage of all available GPUs for hunyuan video avatar rendering with `--multigpu` flag
- **LoRA Support**: Use LoRA models for fine-tuned generation
- **Batch Processing**: Generate multiple videos in sequence
...
### Multi-GPU Support for Hunyuan Video Avatar
...
This will:
- Automatically detect all available CUDA GPUs
- Distribute the hunyuan model components across all GPUs using DataParallel
- Provide faster rendering times for hunyuan video avatar generation
- Fall back to single GPU mode if only one GPU is available

### Other Options
- `--share`: Create a shared URL to access webserver remotely
- `--server-port`: Specify server port (default: 7860)
- `--listen`: Make server accessible on local network
- `--open-browser`: Automatically open browser when starting
- `--theme`: Set UI theme
- `--gpu`: Specify default GPU device
