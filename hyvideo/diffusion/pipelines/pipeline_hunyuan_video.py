...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    def _get_vae(self):
        """Get the actual VAE from DataParallel wrapper if needed"""
        return self._get_model(self.vae)

    def _get_transformer(self):
        """Get the actual transformer from DataParallel wrapper if needed"""
        return self._get_model(self.transformer)

    def _get_text_encoder(self):
        """Get the actual text encoder from DataParallel wrapper if needed"""
        return self._get_model(self.text_encoder)

    def _get_text_encoder_2(self):
        """Get the actual text encoder 2 from DataParallel wrapper if needed"""
        if self.text_encoder_2 is not None:
            return self._get_model(self.text_encoder_2)
        return None
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self._get_transformer().enable_cache:
            teacache_multiplier = self._get_transformer().teacache_multiplier
            self._get_transformer().accumulated_rel_l1_distance = 0
            self._get_transformer().rel_l1_thresh = 0.1 if teacache_multiplier < 2 else 0.15
            # self._get_transformer().cache_start_step =  int(tea_cache_start_step_perc*num_inference_steps/100)
        ...
        # Handle multi-GPU setup - ensure device is properly set for DataParallel models
        if hasattr(self.transformer, 'module'):
            # If using DataParallel, use the first GPU as the primary device
        ...
        if self._get_transformer().mixed_precision:
        ...
        num_channels_latents  = self._get_transformer().config.in_channels
        ...
        if self._get_transformer().enable_cache:
            self._get_transformer().previous_residual = [None] * latent_items
