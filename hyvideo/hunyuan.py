...
class HunyuanVideoSampler(Inference):
    ...
    def generate(
        ...
        **kwargs,
        ...
        if hasattr(self.model, 'module'):  # DataParallel wrapped model
            device = self.model.module.device if hasattr(self.model.module, 'device') else 'cuda'
        else:
        ...
        if VAE_tile_size != None:
            ...
            vae_model.tile_sample_min_tsize = VAE_tile_size["tile_sample_min_tsize"]
            vae_model.tile_latent_min_tsize = VAE_tile_size["tile_latent_min_tsize"]
            vae_model.tile_sample_min_size = VAE_tile_size["tile_sample_min_size"]
            vae_model.tile_latent_min_size = VAE_tile_size["tile_latent_min_size"]
            vae_model.tile_overlap_factor = VAE_tile_size["tile_overlap_factor"]
        ...
        if i2v_mode:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
                vae_model = self.pipeline.vae.module if hasattr(self.pipeline.vae, 'module') else self.pipeline.vae
                img_latents = vae_model.encode(semantic_image_pixel_values).latent_dist.mode() # B, C, F, H, W
                img_latents.mul_(vae_model.config.scaling_factor)
        ...
        if input_video != None or input_frames != None:
            ...
            vae_model = self.vae.module if hasattr(self.vae, 'module') else self.vae
            bg_latents = vae_model.encode(pixel_value_bg).latent_dist.sample()                
            pixel_value_mask = pixel_value_mask.div_(127.5).add_(-1.)             
            mask_latents = vae_model.encode(pixel_value_mask).latent_dist.sample()
            bg_latents = torch.cat([bg_latents, mask_latents], dim=1)
            bg_latents.mul_(vae_model.config.scaling_factor)
        ...
        if self.avatar:
            ...
            align_model = self.align_instance.facedet.model
            align_model.to(device)
            face_masks = get_facemask(pixel_value_ref.to(device)*255, self.align_instance, area=3.0) 
            ...
            vae_model = self.vae.module if hasattr(self.vae, 'module') else self.vae
            vae_dtype = vae_model.dtype
            with torch.autocast(device_type="cuda", dtype=vae_dtype, enabled=vae_dtype != torch.float32):
                ref_latents = vae_model.encode(pixel_value_ref_for_vae).latent_dist.sample()
                ...
                if hasattr(vae_model.config, 'shift_factor') and vae_model.config.shift_factor:
                    ref_latents.sub_(vae_model.config.shift_factor).mul_(vae_model.config.scaling_factor)
                else:
                    ref_latents.mul_(vae_model.config.scaling_factor)

                # out_latents= ref_latents / vae_model.config.scaling_factor
                # image = vae_model.decode(out_latents, return_dict=False, generator=generator)[0]
