...
def setup_multi_gpu_for_hunyuan():
    """Setup multi-GPU environment for hunyuan models"""
    if not args.multigpu:
        return None
    
    num_gpus = get_available_gpus()
    if num_gpus <= 1:
        print("Multi-GPU mode requested but only 1 or 0 GPUs available. Using single GPU mode.")
        return None
    
    print(f"Setting up multi-GPU mode with {num_gpus} GPUs for hunyuan video avatar rendering")
    
    # For single-process multi-GPU usage, we don't need distributed training
    # Just verify CUDA is available and GPUs are accessible
    try:
        # Test access to all GPUs
        for i in range(num_gpus):
            torch.cuda.set_device(i)
            # Create a small tensor to test GPU access
            test_tensor = torch.zeros(1, device=f'cuda:{i}')
            del test_tensor
        
        # Reset to default device
        torch.cuda.set_device(0)
        print(f"Successfully verified access to {num_gpus} GPUs")
...
def _parse_args():
    ...
    parser.add_argument(
        "--multigpu",
        action="store_true",
        help="Enable even usage of all available GPUs for hunyuan video avatar rendering"
    )
    ...
    if model_family == "wan":        
        ...
        model_def = {  
            ...
            "fileList" :[ ["config.json", "special_tokens_map.json", "tokenizer.json", "tokenizer_config.json", "preprocessor_config.json"] + computeList(text_encoder_filename) ,
                          ...
                          ["config.json", "model.safetensors", "preprocessor_config.json", "special_tokens_map.json", "tokenizer.json", "tokenizer_config.json"],
                          ["detface.pt"],
...
def load_hunyuan_model(model_filename, model_type = None,  base_model_type = None, quantizeTransformer = False, dtype = torch.bfloat16, VAE_dtype = torch.float32, mixed_precision_transformer = False, save_quantized = False):
    ...
    num_gpus = setup_multi_gpu_for_hunyuan()
    
    # Disable nccl_gather for multi-GPU to avoid distributed training issues
    nccl_gather = False if num_gpus and num_gpus > 1 else True
    ...
    hunyuan_model = HunyuanVideoSampler.from_pretrained(
        ...
        mixed_precision_transformer = mixed_precision_transformer,
        save_quantized = save_quantized,
        nccl_gather = nccl_gather
    )

    # If multi-GPU is enabled, distribute the model across GPUs
    if num_gpus and num_gpus > 1:
        print(f"Distributing hunyuan model across {num_gpus} GPUs")
        # Use DataParallel for even distribution across all GPUs
        hunyuan_model.model = torch.nn.DataParallel(hunyuan_model.model, device_ids=list(range(num_gpus)))
        if hunyuan_model.vae:
            hunyuan_model.vae = torch.nn.DataParallel(hunyuan_model.vae, device_ids=list(range(num_gpus)))
        if hunyuan_model.text_encoder:
            hunyuan_model.text_encoder = torch.nn.DataParallel(hunyuan_model.text_encoder, device_ids=list(range(num_gpus)))
        if hunyuan_model.text_encoder_2:
            hunyuan_model.text_encoder_2 = torch.nn.DataParallel(hunyuan_model.text_encoder_2, device_ids=list(range(num_gpus)))
